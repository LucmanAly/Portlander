...
| Earnings | boot skeleton | `EmptyState` (0 holdings); separately "No reports match this filter" (0 after filter) | — (per-card `FreshnessLabel` already surfaces provenance age) | Cards with no synced Finnhub data show "no estimate available" per field (`MetricPair`), never fabricated (`BE-01`/`BE-05`) | Local/Demo banner | global banner |
