...
**Current phase:** Phase 1.5 — SnapTrade brokerage sync (live on `main`; connect flow now reaches SnapTrade's real API but is rejected — blocked on owner regenerating SnapTrade API credentials)
**Phase 1 status:** 🟢 Cloud deploy live and verified end-to-end; PR #4 merged. Only formal Phase 1 exit sign-off left. Phase 1.5 (SnapTrade): live on `main`. Five real bugs found and fixed across two rounds of owner click-tests (see Decisions) — the connect flow now correctly reaches SnapTrade's API and correctly surfaces SnapTrade's actual rejection reason in the UI, but SnapTrade itself rejects the request with `401 — "Unable to verify signature sent" (code 1076)`, meaning the `SNAPTRADE_CLIENT_ID`/`SNAPTRADE_CONSUMER_KEY` pair in Supabase secrets doesn't verify. **Blocked on the owner**, not on code — see Blockers.
...
| SnapTrade integration | 🟡 Live on `main`, blocked on owner regenerating SnapTrade credentials | `snaptrade-connect` + `snaptrade-sync` Edge Functions deployed (v10 as of session 9); `holdings.source`/`day_change_*` columns, `snaptrade_users` (lockbox RLS), `snaptrade_connections` tables live; Settings "Brokerage" section + Portfolio table wired up. Five real bugs found and fixed across two rounds of owner click-tests (SDK client construction, `display_name` field, `cost_basis` field, `getAllAccountPositions` response shape, and a client-side bug that discarded the real error body — see Decisions). The flow now correctly reaches SnapTrade's API and surfaces the real rejection reason in the UI: `401 "Unable to verify signature sent" (code 1076)` — the `SNAPTRADE_CLIENT_ID`/`SNAPTRADE_CONSUMER_KEY` pair itself doesn't verify. Added defensive `.trim()` on all secret reads (in case of copy-paste whitespace) — didn't fix it, so the pair is genuinely mismatched, not a formatting artifact. **Owner needs to generate a brand-new Client ID + Consumer Key pair together** in the SnapTrade partner dashboard (not edit the existing ones) and set both fresh in Supabase Secrets — this is the concrete next action, not more code. |
...
1. **Owner:** go to the SnapTrade partner dashboard (portal.snaptrade.com) and generate a **brand-new Client ID + Consumer Key pair together** (don't edit/reuse the existing ones — the current pair fails SnapTrade's own signature check). Set both fresh in Supabase → Project Settings → Edge Functions → Secrets on the Portlander project (`vvstmdnnpjnfvueoecwl`), overwriting `SNAPTRADE_CLIENT_ID`/`SNAPTRADE_CONSUMER_KEY`. Then open Settings on the live site, click "Connect brokerage", complete the SnapTrade portal flow for Fidelity, then "Sync now" — confirm real positions land in Portfolio with `source = Synced`. No redeploy needed after setting secrets. If it still errors, the UI now shows SnapTrade's actual rejection reason directly (this took several rounds of fixes to achieve — see Decisions) — read that message first, it's real, not generic.
...
- **SnapTrade "Connect brokerage" — `401 "Unable to verify signature sent"` (code 1076).** The current `SNAPTRADE_CLIENT_ID`/`SNAPTRADE_CONSUMER_KEY` pair in Supabase Secrets fails SnapTrade's own signature verification. Ruled out: our SDK usage (verified against real SDK source, correct), whitespace/copy-paste contamination (added defensive `.trim()`, no change). What's left is that the pair itself is invalid/mismatched on SnapTrade's side — something only fixable from the SnapTrade partner dashboard. **Owner action needed:** generate a fresh Client ID + Consumer Key pair together and set both in Supabase. No Claude Code session can do this — no MCP tool exposes SnapTrade credential management or Supabase secrets.
...
| 2026-08-01 | Diagnosed SnapTrade `401` as a credential problem, not a code problem, and stopped there | Sequence: "Missing secrets" (owner hadn't set `SNAPTRADE_CLIENT_ID`/`CONSUMER_KEY` on this project) → owner set them → `401` with no detail → added responseBody surfacing → real reason revealed: `{"detail":"Unable to verify signature sent","status_code":401,"code":"1076"}` → added defensive `.trim()` on all secret reads (rules out copy-paste whitespace) → still fails. This error means SnapTrade recognizes the `clientId` but the HMAC computed from `consumerKey` doesn't match what they have on file — a genuinely mismatched/invalid credential pair, not a bug in the SDK call (already verified against real SDK source in an earlier session) or in how the secrets are read. No further code fix is possible without owner action; see Blockers. |
