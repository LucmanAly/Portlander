...
| SnapTrade integration | 🟡 Personal-auth refactor deployed (v17); owner click-test pending | Root cause **confirmed by SnapTrade's own error** (`1012`), not just inferred. Both functions now default to `SNAPTRADE_AUTH_MODE=personal`: `SnaptradeAuth.personalApiKey`, no `registerSnapTradeUser`, no `userId`/`userSecret`, `connectionType: 'read'`. Personal mode is gated to one user (`SNAPTRADE_OWNER_USER_ID`, else the project's single auth user, else refuse) because a Personal key always returns the key owner's own brokerage account. Commercial mode is retained intact behind the switch. No new secrets needed for the current 1-auth-user project. |
...
**Advisory, not a task:** owner asked whether to connect the **DeepSeek API**. Answer written up in `docs/AI.md` — verdict is *yes, but as a nightly batch enrichment worker, gated behind Phase 1 sign-off*, with a hard data boundary (position data never leaves Supabase; enrich the ticker, join locally). Ranked 8 candidate uses; the recommended first build is CSV-import repair, not the flashy one. Nothing built, no API key anywhere. Three open questions for the owner at the end of that doc — the load-bearing one is PRC-hosted first-party API vs a US-hosted endpoint running the same MIT-licensed weights.
...
- **None outstanding on SnapTrade.** The Personal-vs-Commercial mismatch that blocked every prior session is fixed and deployed (v17). What remains is verification, not a blocker: the connect → Fidelity → sync path has never run end-to-end, so treat it as unproven until the owner confirms a real sync.
...
| 2026-08-01 | If an LLM is added, it may never produce a number the UI renders, and never sees position data — it enriches *tickers*, and Postgres joins that to holdings locally | `src/lib/scoring.ts` owns every displayed number, and `AGENTS.md` demands the impact score stay transparent; an LLM-produced figure would break both. The privacy half is forced by SnapTrade: real Fidelity share counts/cost basis now live in `holdings`, and DeepSeek's first-party API is PRC-hosted with no published zero-retention/no-training commitment for API customers. Enriching the ticker and joining locally yields identical UI output while sending nothing sensitive. Corollary: any future job that genuinely needs private text (journal notes) switches *host* — the weights are MIT and hosted in the US by several providers — rather than relaxing the boundary, which is why the plan puts a provider shim in on day one. Full reasoning in `docs/AI.md`. |
...
- Owner asked whether to connect the DeepSeek API and how to maximize its usefulness. **Discussion + plan only — no integration code, no key, no schema change.** SnapTrade state is untouched; session 12's click-test is still the blocker.
- Wrote `docs/AI.md`: verdict, the data boundary, 8 ranked ideas (3 recommended, 2 deferred, 3 explicitly rejected), architecture following the existing `refresh-quotes`/`sync-events` pattern, cost math, and a sequencing plan gated on Phase 1 sign-off.
- Verified rather than recalled: `deepseek-chat`/`deepseek-reasoner` **were deprecated 2026-07-24** — current ID is `deepseek-v4-flash` (1M context, ~$0.14/$0.28 per 1M, ~98% prefix-cache discount). Pricing came from aggregators only; `api-docs.deepseek.com` 403s from this sandbox, so the doc flags it for re-verification before billing.
- Estimated total run cost of all recommended features at **<$3/yr** at 40 tickers — so the decision rests on privacy and product discipline, not price.
- Left three open questions for the owner at the end of `docs/AI.md`; the load-bearing one is host jurisdiction.
- Key files: `docs/AI.md` (new), one Decisions row above.
...
3. **No tool on the Supabase MCP surface exposes secrets management or Auth URL config** — confirmed repeatedly across sessions. Any new secret always needs the owner to paste it into the dashboard themselves. **But `deploy_edge_function` does exist and works** (session 12 shipped v17 of both SnapTrade functions with it, and `get_edge_function` reads the deployed source back to verify). Don't hand the owner a deploy step you can do yourself — that mistake cost several sessions a full round-trip each. Design around it: prefer a sensible default in code over a new secret the owner has to go set.
...
5. **SnapTrade runs in `personal` mode by default** (`SNAPTRADE_AUTH_MODE`), on v17 of both Edge Functions — read `supabase/functions/snaptrade-connect/README.md`'s comparison table before touching either function; the two customer models differ per endpoint and mixing them is what broke sessions 8-11. **The connect→sync flow has still never succeeded end-to-end** — don't assume it works until the owner confirms a real sync. If a *new* error shows up, don't re-guess at SDK usage from docs — download the real tarball from the npm registry (`curl https://registry.npmjs.org/snaptrade-typescript-sdk/latest` for the version, then the tarball URL from that response) and grep the `.d.ts`/`.mjs` directly, the way every real bug here was actually found. Dramatically more reliable than reasoning from documentation or MCP tool names: SnapTrade's own `1012` text says "use the OAuth bearer flow", which sounds like a whole separate thing to build and isn't — the SDK's `personalApiKey` mode already handles it.
