...
**Last agent:** codex
**Current phase:** Phase 1.5 — SnapTrade brokerage sync (live on `main`; connect flow is blocked before first SnapTrade user registration by `401 "Unable to verify signature sent"` code `1076`; diagnosis narrowed to API-key mode/pair — see Blockers)
**Phase 1 status:** 🟢 Cloud deploy live and verified end-to-end; PR #4 merged. Only formal Phase 1 exit sign-off left. Phase 1.5 (SnapTrade): live on `main`; the deployed v10 connect function reaches SnapTrade but fails during the very first `registerSnapTradeUser` call. Supabase is healthy, the request is not rate-limited, and the official SDK v11.0.4 is constructing the signed request. Remaining root-cause fork: (a) a Personal API key is being used by Portlander's Commercial-only registration flow, or (b) the stored Commercial `clientId`/`consumerKey` pair is invalid, stale, or mismatched. Key type cannot be inferred from Supabase because secret values are intentionally unreadable through the connector. **See Blockers for the exact discriminator and fix.**
...
| SnapTrade integration | 🟡 Live on `main`, blocked before first user registration | `snaptrade-connect`/`snaptrade-sync` v10 are active and match the official SDK's Commercial flow, but SnapTrade rejects the initial signed `registerSnapTradeUser` request with `401` code `1076`. Supabase is healthy; logs show the deployed v10 call; rate limit is untouched (250/250); DB counts remain 0 SnapTrade users/connections/holdings. Root cause is now narrowed to API-key identity: Personal key used in Commercial code, or invalid/stale/mismatched Commercial pair. |
...
1. **Owner:** on the SnapTrade Dashboard API Key page, identify whether the current key is explicitly labeled **Personal** or **Commercial/Test** (do not paste either secret). If Personal, Portlander must switch to `SnaptradeAuth.personalApiKey`, skip `registerSnapTradeUser`, and omit `userId`/`userSecret` throughout connect/sync. If Commercial/Test, the code path is already correct; contact SnapTrade support to validate/reset the consumer key, supplying request ID `5c7770169c3395b12137f3e820fd579d` and the `2026-08-01 00:19:17 UTC` failure. SnapTrade's own FAQ says code `1076` under an official SDK most likely means an invalid consumer key. Re-enter both credentials in Supabase only after confirming they belong to the same labeled key record.
...
- **SnapTrade "Connect brokerage" — `401 "Unable to verify signature sent"` (code 1076) during the first `registerSnapTradeUser` call.** Proven non-causes: Fidelity/brokerage login (portal is never reached), Supabase health/auth (function executes normally), rate limiting (250/250 remaining), missing secrets (the SDK signs and SnapTrade compares a signature), stale deployment (v10 is active), and hand-written signing (official SDK v11.0.4 is used). Remaining discriminator: check the key label in SnapTrade Dashboard. **Personal** credentials are incompatible with the current Commercial-only flow and require a code refactor; **Commercial/Test** credentials mean the current code path is correct and SnapTrade must validate/reset the consumer key or the exact matched pair must be re-entered. Supabase's connector cannot read back secret values or determine their SnapTrade key type by design.
...
| 2026-08-01 | Refined SnapTrade `401` diagnosis from "bad pair" to an API-key identity fork | Direct inspection confirmed the deployed v10 code uses the latest official SDK v11.0.4 correctly for **Commercial** auth and fails on the first `registerSnapTradeUser` request. Current SnapTrade docs add a material discriminator the earlier diagnosis omitted: Personal keys must use `personalApiKey`, must not register users, and omit `userId`/`userSecret`. If the stored key is Commercial/Test, SnapTrade's FAQ makes an invalid/stale/mismatched consumer key the remaining likely cause and instructs contacting staff for reset. |
